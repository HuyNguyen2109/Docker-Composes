# VM/VPS Setup Script Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create `scripts/vm-setup.sh`, a single self-contained Bash script that provisions a fresh VM/VPS by detecting the OS, running a full system update/upgrade, and installing all user-installed tools with a combined TUI (spinner + progress bar + colored headers + ✓/✗).

**Architecture:** A single Bash 4+ file built around a `run_cmd()` wrapper that handles a background spinner, stderr/stdout capture, and error logging. All install functions follow a consistent pattern: skip-if-already-installed, per-OS-type install block, status/version tracking. An `INSTALL_STATUS` and `INSTALL_VERSION` associative array are populated by each function and rendered into a final summary table. 16 sections run sequentially; only system update/upgrade is fatal on failure.

**Tech Stack:** Bash 4+, shellcheck 0.10+, curl, gnupg, coreutils (mktemp, printf, date, sleep)

**Spec:** `docs/superpowers/specs/2026-05-21-vm-setup-script-design.md`
...
|---|---|---|
| `scripts/vm-setup.sh` | Create | Complete setup script — all logic in one file |
| `setup-YYYYMMDD-HHMMSS.log` | Created at runtime | Error log written to same dir as script |

---

## Task 1: Skeleton — shebang, env-var comments, constants, TUI banner/header/progress
...
```bash
cat > /root/workspace/My-DevOps/scripts/vm-setup.sh << 'ENDOFSCRIPT'
#!/usr/bin/env bash
# =============================================================================
...
draw_progress_bar() {
  ...
  printf "  ${BLUE}%s${GRAY}%s${RESET}  ${BOLD}%3d%%${RESET}  [%d/%d sections]\n" \
    "$filled_str" "$empty_str" "$pct" "$current" "$TOTAL_SECTIONS"
}
ENDOFSCRIPT
```
...
```bash
bash -n /root/workspace/My-DevOps/scripts/vm-setup.sh
```
...
git add scripts/vm-setup.sh
git commit -m "feat(scripts): add vm-setup.sh skeleton — constants, TUI banner/header/progress"
```

---

## Task 2: Spinner, run_cmd, log_error, cleanup trap, check_root

**Files:**
- Modify: `scripts/vm-setup.sh` (append after `draw_progress_bar`)

- [ ] **Step 1: Append execution utilities**
...
cd /root/workspace/My-DevOps
git add scripts/vm-setup.sh
git commit -m "feat(scripts): add spinner, run_cmd, log_error, cleanup trap, check_root"
```

---

## Task 3: OS detection — detect_os()

**Files:**
- Modify: `scripts/vm-setup.sh` (append)

- [ ] **Step 1: Append detect_os function**
...
- [ ] **Step 3: Smoke-test OS detection on current machine**

```bash
# Source just enough of the script to run detect_os in isolation
bash -c '
  source /root/workspace/My-DevOps/scripts/vm-setup.sh 2>/dev/null
  detect_os
' 2>/dev/null | grep -E "(OS|Arch|Distribution|Package)"
```

Expected output (on this Debian 13 machine):
```
  ✓ OS              : Debian GNU/Linux 13 (trixie)
  ✓ Architecture    : x86_64
  ✓ Distribution ID : debian (trixie)
  ✓ Package Manager : apt
```
...
cd /root/workspace/My-DevOps
git add scripts/vm-setup.sh
git commit -m "feat(scripts): add detect_os() — all 4 distro families"
```

---

## Task 4: System update/upgrade + base packages

**Files:**
- Modify: `scripts/vm-setup.sh` (append)

- [ ] **Step 1: Append system_update and install_base**
...
cd /root/workspace/My-DevOps
git add scripts/vm-setup.sh
git commit -m "feat(scripts): add system_update() and install_base()"
```

---

## Task 5: Docker installation (official docs)

**Files:**
- Modify: `scripts/vm-setup.sh` (append)

- [ ] **Step 1: Append install_docker**
...
cd /root/workspace/My-DevOps
git add scripts/vm-setup.sh
git commit -m "feat(scripts): add install_docker() — official Docker docs method"
```
...
- [ ] **Step 1: Append install_kubectl and install_helm**
...
install_helm() {
  ...
  run_cmd "Downloading Helm install script" bash -c "
    curl -fsSL https://raw.githubusercontent.com/helm/helm/main/scripts/get-helm-3 \
      -o /tmp/get-helm-3.sh
    chmod +x /tmp/get-helm-3.sh
  " || true
  run_cmd "Installing Helm" bash /tmp/get-helm-3.sh || true
...
cd /root/workspace/My-DevOps
git add scripts/vm-setup.sh
git commit -m "feat(scripts): add install_kubectl() and install_helm()"
```

---

## Task 7: Azure CLI + HashiCorp Vault

**Files:**
- Modify: `scripts/vm-setup.sh` (append)

- [ ] **Step 1: Append install_azure_cli and install_vault**
...
install_azure_cli() {
  ...
  case "${OS_TYPE}" in
    debian)
      ...
      run_cmd "Downloading Microsoft install script" bash -c "
        curl -fsSL https://aka.ms/InstallAzureCLIDeb -o /tmp/azure-cli-install.sh
        chmod +x /tmp/azure-cli-install.sh
      " || true
      run_cmd "Running Azure CLI installer" bash /tmp/azure-cli-install.sh || true
...
install_vault() {
  ...
  case "${OS_TYPE}" in
    ...
    rhel)
      ...
      ;;
    arch|suse)
      # Download latest binary directly from HashiCorp releases
      run_cmd "Installing Vault binary from HashiCorp releases" bash -c "
        ...
        curl -fsSL \"https://releases.hashicorp.com/vault/\${VAULT_VER}/vault_\${VAULT_VER}_linux_\${ARCH_NAME}.zip\" \
          -o /tmp/vault.zip
...
cd /root/workspace/My-DevOps
git add scripts/vm-setup.sh
git commit -m "feat(scripts): add install_azure_cli() and install_vault()"
```

---

## Task 8: Node.js LTS + npm globals + Python 3

**Files:**
- Modify: `scripts/vm-setup.sh` (append)

- [ ] **Step 1: Append install_node, install_npm_globals, install_python**
...
install_node() {
  ...
  case "${OS_TYPE}" in
    debian)
      run_cmd "Downloading NodeSource LTS setup" bash -c "
        curl -fsSL https://deb.nodesource.com/setup_lts.x -o /tmp/nodesource-setup.sh
        chmod +x /tmp/nodesource-setup.sh
      " || true
      run_cmd "Running NodeSource setup" bash /tmp/nodesource-setup.sh || true
      run_cmd "Installing nodejs" apt-get install -y nodejs || true
      rm -f /tmp/nodesource-setup.sh
      ;;
    rhel)
      run_cmd "Downloading NodeSource LTS setup" bash -c "
        curl -fsSL https://rpm.nodesource.com/setup_lts.x -o /tmp/nodesource-setup.sh
        chmod +x /tmp/nodesource-setup.sh
      " || true
      run_cmd "Running NodeSource setup" bash /tmp/nodesource-setup.sh || true
...
cd /root/workspace/My-DevOps
git add scripts/vm-setup.sh
git commit -m "feat(scripts): add install_node(), install_npm_globals(), install_python()"
```

---

## Task 9: PostgreSQL client + Redis tools + Common tools

**Files:**
- Modify: `scripts/vm-setup.sh` (append)

- [ ] **Step 1: Append install_pg_client, install_redis_tools, install_common_tools**
...
cd /root/workspace/My-DevOps
git add scripts/vm-setup.sh
git commit -m "feat(scripts): add install_pg_client(), install_redis_tools(), install_common_tools()"
```

---

## Task 10: User/group setup + summary report + reboot prompt + main()

**Files:**
- Modify: `scripts/vm-setup.sh` (append — final sections)

- [ ] **Step 1: Append setup_users, print_summary, reboot_prompt, main**
...
print_summary() {
  ...
  for tool in "${tools[@]}"; do
    ...
    case "${status_val}" in
      ...
      skipped*|"skipped (root-only session)")
...
cd /root/workspace/My-DevOps
git add scripts/vm-setup.sh
git commit -m "feat(scripts): add setup_users(), print_summary(), reboot_prompt(), main()"
```

---

## Task 11: shellcheck validation and fixes

**Files:**
- Modify: `scripts/vm-setup.sh` (fix any shellcheck warnings)

- [ ] **Step 1: Run shellcheck**

```bash
shellcheck -s bash /root/workspace/My-DevOps/scripts/vm-setup.sh
```

Expected output: zero warnings. If warnings appear, address each one as described below.

**Common shellcheck fixes:**

- **SC2155** (`local foo=$(cmd)` masks return value):
  Replace `local var=$(cmd)` with two lines:
  ```bash
  local var
  var=$(cmd)
  ```
...
- **SC2086** (unquoted variable):
  Replace `$VAR` with `"${VAR}"` in all string contexts.

- **SC2064** (use single quotes in trap for static string):
  Replace `trap "cleanup" EXIT` with `trap 'cleanup' EXIT INT TERM HUP`
  *(already correct in Task 2, verify it stayed that way)*

- **SC1091** (cannot source dynamic file):
  Verify `# shellcheck source=/dev/null` directive is directly above the `source /etc/os-release` line.

- [ ] **Step 2: Run shellcheck again to confirm zero warnings**
...
cd /root/workspace/My-DevOps
git add scripts/vm-setup.sh
git commit -m "fix(scripts): address all shellcheck warnings in vm-setup.sh"
```

---

## Task 12: Make executable, smoke-test, final commit

**Files:**
- Modify: `scripts/vm-setup.sh` (chmod only)

- [ ] **Step 1: Make the script executable**

```bash
chmod +x /root/workspace/My-DevOps/scripts/vm-setup.sh
```

- [ ] **Step 2: Verify shebang and executability**

```bash
head -1 /root/workspace/My-DevOps/scripts/vm-setup.sh
ls -la /root/workspace/My-DevOps/scripts/vm-setup.sh
```

Expected:
```
#!/usr/bin/env bash
-rwxr-xr-x  1 root root  ...  scripts/vm-setup.sh
```

- [ ] **Step 3: Smoke-test — bash -n (full syntax check on final file)**

```bash
bash -n /root/workspace/My-DevOps/scripts/vm-setup.sh && echo "SYNTAX OK"
```
...
- [ ] **Step 4: Smoke-test — verify OS detection works in isolation**

```bash
# Source only up to detect_os() and call it directly (no root required)
bash -c '
  BASH_VERSINFO=([0]=5)
  source /root/workspace/My-DevOps/scripts/vm-setup.sh 2>/dev/null || true
...
- [ ] **Step 5: Verify line count is reasonable (should be 400-700 lines)**

```bash
wc -l /root/workspace/My-DevOps/scripts/vm-setup.sh
```

Expected: between 400 and 750 lines.
...
cd /root/workspace/My-DevOps
git add scripts/vm-setup.sh
git commit -m "feat(scripts): vm-setup.sh complete — make executable, all checks pass

Full VM/VPS provisioning script supporting Debian/Ubuntu, RHEL/Fedora,
Arch Linux, and openSUSE. Installs: Docker (official), kubectl, helm,
azure-cli, vault, Node.js LTS, npm globals, python3, postgresql-client,
redis-tools, and common tools. Combined TUI with spinner + progress bar
+ colored headers + check/cross per command.
...
**Spec coverage check:**
- ✅ Env var comment header (Task 1, lines 8-21)
- ✅ OS detection + arch + distro + PM detection (Task 3)
- ✅ Full update/upgrade + dist-upgrade + cleanup (Task 4)
- ✅ Base packages distro-mapped (Task 4)
- ✅ Docker via official docs (Task 5)
- ✅ kubectl via official k8s repo (Task 6)
- ✅ Helm via get-helm-3 script (Task 6)
- ✅ Azure CLI via official Microsoft installer (Task 7)
- ✅ Vault via official HashiCorp repo / binary fallback (Task 7)
- ✅ Node.js via NodeSource LTS (Task 8)
- ✅ npm globals with @latest (Task 8)
- ✅ Python3 + pip + dev + venv (Task 8)
- ✅ PostgreSQL client via PGDG repo (Task 9)
- ✅ Redis tools (Task 9)
- ✅ Common tools all distro-mapped (Task 9)
- ✅ Docker group assignment for SUDO_USER (Task 10)
- ✅ Combined TUI: spinner + filled bar + colored headers + ✓/✗ (Tasks 1-2)
- ✅ Error logging to timestamped .log file (Task 2)
- ✅ Individual package failures non-fatal, system update fatal (Tasks 2, 4)
- ✅ Summary report box with version + status per tool (Task 10)
- ✅ Reboot prompt y/N (Task 10)
- ✅ Latest version always (no version pins anywhere)
